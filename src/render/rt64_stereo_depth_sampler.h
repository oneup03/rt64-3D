...
namespace RT64 {
    // Reads back a sparse grid of depth-buffer patches, for depth-driven
    // auto-convergence (dynamic3d 4.1).
    //
    // The copy is queued on the render thread and read back several frames later
    // out of a ring of readback buffers, rather than stalling the GPU the way
    // RT64's framebuffer-change readback does (end/execute/wait/begin). A
    // per-frame full pipeline flush would cost more than this feature is worth,
    // and a few frames of latency is invisible on a value that is temporally
    // smoothed anyway.
    //
    // Two statistics, two sample windows, deliberately not shared. The
    // convergence loop wants a NEAR statistic over a wide region of interest;
    // the depth-aware crosshair (dynamic3d 5.1) wants a MEDIAN over a tight
    // window centred on the aim point. Reading the aim depth out of the wide
    // grid would draw from a fifth of the screen's width, so the aim patch is
    // copied separately and sits after the grid in the same buffer.
    //
    ...
    struct StereoDepthSampler {
        ...
        // bottom margin is much larger than the top: geometry that is
        // permanently close to the camera sits low in frame, because that is
        // where a game puts things attached to the viewer, while the top is sky
        // and distant scenery — exactly what the loop wants to see. Raising
        // RoiMarginBottomPercent excludes more low-frame clutter at the cost of
        // ignoring genuinely near floor when looking down.
        ...
        // Grid spanning the usable region. 45 patches of 32x32 is ~46k depth
        // texels per frame — still a small fraction of a 4K depth target, and
        // dense enough that a near object anywhere in frame lands in several of
        // them rather than being missed between samples.
        ...
        // The aim patch is one more of the same, appended after the grid, so the
        // copy and the readback keep a single uniform stride.
        static constexpr uint32_t AimPatchIndex = PatchCount;
        static constexpr uint32_t TotalPatchCount = PatchCount + 1;
        ...
        struct Slot {
            ...
            bool queued = false;
            // Whether this slot's aim patch was taken at a reported reticle
            // position. Recorded per slot rather than on the sampler because the
            // readback is several frames behind: by the time it is read, the
            // reticle may have come or gone.
            bool aimSampled = false;
        };

        std::array<Slot, SlotCount> slots;
        uint32_t frameIndex = 0;

        // Queue a copy of this frame's depth patches. Safe to call with a null or
        // multisampled target; it simply does nothing.
        //
        ...
        struct Sample {
            ...
            // Depth under the aim point, for the crosshair. A MEDIAN, not the
            // percentile used above: the crosshair wants the depth of whatever
            // fills the reticle, and the reticle is small enough that a low
            // percentile would bias toward whatever edge of it happens to
            // overlap something nearer.
            bool aimValid = false;
            float aimDeviceDepth = -1.0f;
        };

        // Read back the oldest completed copy.
        Sample fetch();
    };
    ...
    struct StereoAutoConvergence {
        ...
        // cut stays changed; a small object flickering in and out of a patch does
        // not, so the snap only fires once this persists.
        ...
        // Returns the convergence to apply, in the same tenths unit, which is the
        // manual value when there is nothing to protect against.
        uint32_t update(float nearestViewZ, uint32_t manualConvergenceTenths, uint32_t separationSlider,
                        int32_t comfortTarget);
